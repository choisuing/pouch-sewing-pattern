...
**도안 이름**은 `사각사각 지퍼 파우치 270*140*100` 형식으로 `앞판` 한가운데에 한 번 찍고, 그 아래 한 줄 작고 옅게 출처(`예쁘게 만들어보세요! @silsuni_lab`)를 남긴다. 종이만 따로 돌아다녀도 어디서 나왔는지 알 수 있다. 문구는 `src/core/dimensions.ts`의 `WATERMARK`에 있고, 바꾸면 서브셋 폰트를 다시 만들어야 한다. 전개도에서 가장 넓게 비어 있고 골선으로 절반만 남겨도 살아 있는 자리다. 치수 순서는 화면·라벨과 같은 가로\*높이\*바닥폭이다.
